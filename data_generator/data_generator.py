...
def create_dictionary(level_of_nesting: int, max_number_of_keys: int, max_word_length: int, key_names: List[tuple],
                      current_level) -> dict:
    ...
    for _ in range(num_of_keys):
        ...
        if current_level < level_of_nesting and random() < 0.35:
            dictionary[key_w_type[0]] = create_dictionary(level_of_nesting, max_number_of_keys, max_word_length,
                                                          key_names, current_level+1)
        else:
...
def generated_key_value_pairs(number_of_lines: int, level_of_nesting: int, max_number_of_keys: int,
                              max_word_length: int, key_names: List[tuple]) -> List[dict]:
    ...
    for it in range(number_of_lines):
        records.append({f'person_{it}': create_dictionary(level_of_nesting, max_number_of_keys, max_word_length,
                                                          key_names, 0)})
